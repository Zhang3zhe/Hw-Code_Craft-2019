...
def schedule_running_cars(car_run_list, road_list, cross_list, set_time):
        ...
        while cur != None:
            ...
            if cur.prev_car_on_channel == None:
                ...
                if cur.prev_car_on_channel.position - cur.position == 1:
                    ...
                    #cur.speed_now = min(cur.speed, cur.channel_ptr.speed)   #这个车速会不会有问题?比如造成追尾
                    ###########################################################
                    cur.speed_now = min(cur.speed, cur.channel_ptr.speed,cur.prev_car_on_channel.speed_now+(cur.prev_car_on_channel.position - cur.position-1))
                    ###########################################################
            #若车的运行时间未耗尽
            if cur.time_out == 0:
                #重置nowhere标志位
                ...
                if cur.prev_car_on_channel != None:
                    ...
                    if dis_prev_car == 1:
                        cur.dis_before_switch += 0
                        cur.nowhere = 1                       #######################   nowhere是不是现在呆在这儿的意思
                    #若大于1
                    else:
                        ...
                        if dis_prev_car <= cur.speed_now:
                            ...
                            print('------>',cur.id, cur.position, cur.channel, cur.channel_ptr.road_id)
                            cur.nowhere = 1
                ...
                #若是channel上为第一辆车
                    ...
                    #若不在目的道路上      ??????????????????
                        ...
                        #若已超出，记录切换road之前进过多少距离，所花费的vruntime
                            ...
                            channel_switch = cur.to_ptr.channel_ptr           #######################################切换
                            ...
                            #若有可用channel，计算在该channel上能移动的距离cur.dis_after_switch
                                ...
                                #若不是第一辆车
                                if channel_switch.car_ptr != None:                             ####################这个地方属于关键车速
                                    ...
                                    #若dis_after_switch大于0，则说明可以通过路口
                                        ...
                                        print('------>', cur.id, cur.position, cur.channel, cur.channel_ptr.road_id)
                                        if cur.prev_car_on_channel.position - cur.position == 1:    ############################ None Exception
                                            cur.speed_now = min(cur.prev_car_on_channel.speed_now, cur.speed)
                                ...
                                #若是第一辆车
                                    ...
                                    ###########################################################
                                    dis_after_switch = min(cur.speed, channel_switch.speed) - dis_before_switch        # cur.dis_before_switch与dis_before_switch是不一样的,此处修改：去掉cur
                                    # 若dis_after_switch小于0，则说明不能通过路口，情况同没有channel可用一样
                                    if dis_after_switch <= 0:
                                        cur.vruntime = cur.vruntime + vruntime_before_switch
                                        cur.dis_before_switch += cur.from_ptr.length - cur.position
        ...
        if cur.set_time <= set_time:
            ...
            if channel_on:
                ...
                if cur.prev_car_on_channel != None:
                    if cur.prev_car_on_channel.position - cur.position == 1:
                        cur.speed_now = min(cur.prev_car_on_channel.speed_now, cur.speed)
                        cur.position = max(0, min(channel_on.position - 1, cur.speed_now))################################
                    ...
                    cur.position = max(0, min(channel_on.position - 1, cur.speed_now))############################## 1---->0
                    print('------>', cur.id, cur.position, cur.channel, cur.channel_ptr.road_id)
    ...
    if print_flag:
        ...
        if cur == None:
            print('no car', ' ')
        while cur != None:
            print(cur.id, cur.position,cur.channel,cur.channel_ptr.road_id)#, cur.vruntime, cur.channel_ptr, cur.prev_car_on_channel, cur)
            cur = cur.car_next
        print('running...')
        cur = in_gara_car_list.head
        if cur == None:
